...
export type NeoMaterials = {
  ...
  /** نقابِ صورت — تنها سطحِ آینه‌ای، چشم‌ها از داخلش می‌تابند */
...
export function buildNeoBot(m: NeoMaterials): NeoParts {
   ...
   */
  /**
   * نقاب — نه یک صفحهٔ جدا، یک **تکه از خودِ جمجمه**.
   *
   * نسخهٔ اولِ این صفحه یک ورقهٔ مسطح جلوی سر بود و همان بود که
   * ماسکی و چسبانده به نظر می‌رسید؛ برای همین هم حذف شد. این یکی
   * فرق دارد: از همان کره با همان شعاع بریده شده، فقط کمی بیرون‌تر.
   * یعنی سیلوئت ذره‌ای عوض نمی‌شود و لبه‌اش هیچ‌جا از سر بیرون
   * نمی‌زند — تنها چیزی که عوض می‌شود جنسِ سطح است.
   *
   * `phi` از همان نقطه‌ای شروع می‌شود که چشم‌ها رویش نشسته‌اند
   * (صفرِ کره روی −X است، نه +Z) و `theta` فقط تا زیرِ خطِ چشم
   * می‌آید: پیشانی و چانه پوستهٔ مات می‌مانند، مثلِ مرجع.
   */
  {
    const visor = mesh(
      new THREE.SphereGeometry(0.502, 48, 32, Math.PI / 2 - 0.72, 1.44, 0.62, 0.72),
      m.visor,
      head,
    );
