 ...
 * کدِ لایسنس، تکه‌تکه.
 *
 * یک رشتهٔ بلندِ بی‌فاصله را آدم موقع تایپ کردن گم می‌کند و موقع
 * خواندن از روی کاغذ بیشتر. شکستنش هر چهار نویسه همان کاری است که
 * روی کارت بانکی می‌کنند، به همان دلیل. فاصله‌ها فقط دیداری‌اند —
 * در انتخاب و کپی، متنِ اصلی بیرون می‌آید.
 */
const keyBlocks = (k) =>
  String(k)
    .replace(/\s+/g, "")
...
<main class="sheet">
  ...
  <section class="key">
    ...
    <p class="key-code">${esc(keyBlocks(license))}</p>
  ...
  <div class="cols">
    <section class="col">
      <h2>فعال‌سازی</h2>
      <ol class="steps">
        <li><strong>اسپات‌پلیر را نصب کن.</strong> از <code>spotplayer.ir</code>، نسخهٔ ویندوز، مک، لینوکس، اندروید یا iOS. اگر هیچ‌کدام را نمی‌خواهی نصب کنی، نسخهٔ تحت‌وب هم دارد.</li>
  ...
  <section class="notes">
    <h2>سه چیز که بعداً به کارت می‌آید</h2>
    <ul>
      ...
      <li><strong>اگر سهمیه تمام شد، پیام بده.</strong> ریست‌کردنش کار دارد ولی شدنی است — فقط از قبل بگو، نه بعد از اینکه گیر کردی.</li>
    </ul>
  </section>
