...
## 🚀 حالت سریع: بلوک‌های چهارتایی

اگر می‌خوای چهارتا چهارتا بسازی، **کل بلوک زیر رو یکجا کپی کن**. سبک مشترک
داخل خودش هست، پس لازم نیست چیزی اضافه کنی.

بلوک‌ها به ترتیب اولویت چیده شدن. اگر وقت کم داری، فقط بلوک ۱ تا ۳ رو بساز.

---

### بلوک ۱ · کاور مقاله‌های ۱ تا ۴

```
Generate 4 separate images, each in 16:9 aspect ratio.

Shared style for all four: minimal editorial vector illustration, flat geometric
shapes with subtle paper grain, warm cream background #FAF6F1, near-black ink
#1a1714, generous negative space, calm and confident, swiss design influence,
soft long shadows, no text, no letters, no words, no photorealism, no 3d render,
no glossy chrome, no stock photo people, centered composition.

Image 1, accent color violet #7c3aed: a human hand and a geometric robotic hand
reaching toward the same design tool, neither one dominating, balanced composition.

Image 2, accent color deep violet #6d28d9: an abstract funnel turning scattered
chaotic small shapes into one clean ordered rectangle, precision and clarity.

Image 3, accent color blue #1d4ed8: an abstract chat interface panel floating,
with a soft pulsing dot indicating thinking, and a subtle question mark shape
formed by negative space suggesting uncertainty.

Image 4, accent color red #dc2626: an abstract vertical form with stacked input
field rectangles, one field visibly broken and tilted out of alignment while the
rest stay neatly aligned.
```

ذخیره به ترتیب در `public/images/articles/` با نام‌های:
`ai-and-designer-job.jpg` · `prompting-for-designers.jpg` · `designing-ai-interfaces.jpg` · `form-design-mistakes.jpg`

---

### بلوک ۲ · کاور مقاله‌های ۵ تا ۸
...
Image 1, accent color teal #0d9488: modular building blocks snapping together
into an organized grid system, clean modular logic.

Image 2, accent color orange #ea580c: overlapping translucent color swatch circles
forming a controlled palette ladder from light to dark.

Image 3, accent color emerald #059669: an abstract eye shape and a ripple of sound
waves merging into one symmetric mark, inclusive and calm.

Image 4, accent color pink #db2777: three portfolio cards fanned out, the front one
clearly sharper and more refined than the two behind it, quality over quantity.
```

ذخیره در `public/images/articles/`:
`design-system-guide.jpg` · `color-in-ui.jpg` · `accessibility-basics.jpg` · `portfolio-guide.jpg`

---

### بلوک ۳ · دو مقاله‌ی آخر + کاور چک‌لیست + تصویر اشتراک‌گذاری
...
Image 1, 16:9, accent color amber #ca8a04: two abstract speech bubbles facing each
other, one much larger and listening, a small magnifier resting inside it.

Image 2, 16:9, accent color blue #2563eb: an abstract stepped path leading from a
small dot up to an open doorway shape, the beginning of a career.

Image 3, 3:2, accent color violet #7c5cfc: an abstract winding path made of six
connected geometric segments, each segment slightly more refined than the previous
one, ending in a small flag, a sense of ordered progress.

Image 4, wide 1200x630 banner, accent color violet #7c5cfc: a wide minimal
composition with a single bold geometric mark centered on warm cream, generous
empty space all around it, editorial poster feeling.
```

ذخیره:
`public/images/articles/user-research-basics.jpg` · `public/images/articles/first-design-job.jpg` · `public/images/checklist-hero.jpg` · `public/og.jpg`

---

### بلوک ۴ · گواهی + دو فایل رایگان + مرحله‌ی اول چک‌لیست
...
Image 1, 4:3, accent color violet #7c5cfc: an abstract certificate document shown
at a slight angle with a seal badge shape in one corner and a small barcode like
verification mark, official but modern and minimal.

Image 2, square 1:1, accent color violet #7c5cfc: fifty small abstract ui style
thumbnails arranged in a dense neat grid, each one a slightly different visual
treatment.

Image 3, square 1:1, accent color emerald #16a34a: a tall stack of small abstract
command cards seen from a slight angle, suggesting a large organized library.

Image 4, square 1:1, accent color red #dc2626: a hand placing the very first
foundation block on empty ground.
```

ذخیره:
`public/images/certificate-sample.jpg` · `public/images/free/50-styles.jpg` · `public/images/free/700-chatgpt.jpg` · `public/images/checklist/foundation.jpg`

---

### بلوک ۵ · مراحل چک‌لیست (اختیاری)

```
Generate 4 separate images, each square 1:1.
...
Image 1, accent color orange #ea580c: an abstract layout where spacing and
hierarchy are visibly measured by thin guide lines.

Image 2, accent color blue #1d4ed8: a branching flow diagram of simple nodes with
one path highlighted as the chosen route.

Image 3, accent color teal #0d9488: identical modular components arranged in a
strict repeating grid.

Image 4, accent color violet #7c5cfc: several screens of one product connected in
sequence forming a complete flow.
```

ذخیره در `public/images/checklist/`:
`visual.jpg` · `ux.jpg` · `system.jpg` · `project.jpg`

---

### بلوک ۶ · آخرین مرحله (تک)

```
Generate 1 image, square 1:1, accent color pink #db2777: an open portfolio spread
with one project presented prominently.

Style: minimal editorial vector illustration, flat geometric shapes with subtle
paper grain, warm cream background #FAF6F1, near-black ink #1a1714, generous
negative space, swiss design influence, soft long shadows, no text, no letters,
no words, no photorealism, no 3d render, no stock photo people.
```

ذخیره در `public/images/checklist/portfolio.jpg`

> اگر جای خالی این بلوک اذیتت می‌کنه، همراهش سه تا از تصویرهایی که قبلاً خوب
> در نیومدن رو دوباره بساز.
